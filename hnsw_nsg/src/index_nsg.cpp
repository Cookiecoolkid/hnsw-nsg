#include "../include/nsg/index_nsg.h"
...
#include "../include/nsg/exceptions.h"
#include "../include/nsg/parameters.h"
...
void IndexNSG::SearchFromEnterpoint(const float *query, const float *x, size_t K, const Parameters &parameters, 
                                  unsigned *indices, unsigned enterpoint_id) const {
  const unsigned L = parameters.Get<unsigned>("L_search");
  std::vector<Neighbor> retset(L + 1);
  std::vector<unsigned> init_ids(L);
  boost::dynamic_bitset<> flags{nd_, 0};
...
void IndexNSG::MySearch(const float *query, const float *x, size_t K,
                      const Parameters &parameters, unsigned *indices) const {
  const unsigned L = parameters.Get<unsigned>("L_search");
  // data_ = x;
  std::vector<Neighbor> retset(L + 1);
  std::vector<unsigned> init_ids(L);

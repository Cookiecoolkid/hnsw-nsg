...
void search_nsg_cluster(
    ...
    for (const auto& [global_id, dist] : local_results) {
        if (local_queue.size() < static_cast<size_t>(ctx.k)) {
    ...
    if (cluster_min_dist >= local_max_dist && local_queue.size() >= static_cast<size_t>(ctx.k)) {
...
std::vector<unsigned> process_search_results(
    ...
    // 提取前k个结果
    for (size_t i = 0; i < k && i < sorted_results.size(); i++) {
